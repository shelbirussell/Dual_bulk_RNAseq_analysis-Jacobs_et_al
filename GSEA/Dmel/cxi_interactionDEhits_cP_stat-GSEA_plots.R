...
write.table(expressed_ids, "cxi.DE_p1e-64_FBGNids.txt")
...
write.csv(as.data.frame(gsea_kegg), "cxi_interaction_WaldTest-DE_GSEA_gseKEGG_results.p0.5.csv")

### plot results
# Check if gsea_kegg contains any results before plotting
if (!is.null(gsea_kegg) && nrow(gsea_kegg@result) > 0) {
        pdfGseKEGG <- file.path(paste0("cxi_interaction_WaldTest-DE", ".gseKEGG.dotplot.p0.5.pdf"))
        pdf(pdfGseKEGG)
        ...
        for (i in seq_along(gsea_kegg$ID)) {
                pdfGse2KEGG <- file.path(paste0("cxi_interaction_WaldTest-DE", ".geneset", i, ".gseKEGG.plot2.p0.5.pdf"))
                pdf(pdfGse2KEGG)
        ...
        pdfGse2KEGG <- file.path(paste0("cxi_interaction_WaldTest-DE", ".geneset.1-", length(gsea_kegg$ID), ".gseKEGG.plot2.p0.5.pdf"))
        pdf(pdfGse2KEGG)
        ...
        pdfGseKEGGridge <- file.path(paste0("cxi_interaction_WaldTest-DE", ".gseKEGG.ridgeplot.p0.05.pdf"))
        pdf(pdfGseKEGGridge)
        print(ridgeplot(gsea_kegg))
        dev.off()

        gsea_kegg2 <- pairwise_termsim(gsea_kegg)
        pdfGseKEGGemap <- file.path(paste0("cxi_interaction_WaldTest-DE", ".gseKEGG.emapplot.p0.05.pdf"))
        pdf(pdfGseKEGGemap)
...
# Save the results to CSV
write.csv(as.data.frame(gsea_kegg), "cxi_interaction_WaldTest-DE_GSEA_gseKEGG_results.csv")

### plot results
# Check if gsea_kegg contains any results before plotting
if (!is.null(gsea_kegg) && nrow(gsea_kegg@result) > 1) {
        pdfGseKEGG <- file.path(paste0("cxi_interaction_WaldTest-DE", ".gseKEGG.dotplot.pdf"))
        pdf(pdfGseKEGG)
        print(dotplot(gsea_kegg, showCategory = 20))
        dev.off()

        for (i in seq_along(gsea_kegg$ID)) {
                pdfGse2KEGG <- file.path(paste0("cxi_interaction_WaldTest-DE", ".geneset", i, ".gseKEGG.plot2.pdf"))
                pdf(pdfGse2KEGG)
                print(gseaplot2(gsea_kegg, geneSetID = i, title=gsea_kegg$Description[i]))
                dev.off()
        ...
        ## https://rdrr.io/bioc/enrichplot/man/ridgeplot.html
        pdfGseKEGGridge <- file.path(paste0("cxi_interaction_WaldTest-DE", ".gseKEGG.ridgeplot.pdf"))
        pdf(pdfGseKEGGridge)
        print(ridgeplot(gsea_kegg))
        dev.off()

        gsea_kegg2 <- pairwise_termsim(gsea_kegg)
        pdfGseKEGGemap <- file.path(paste0("cxi_interaction_WaldTest-DE", ".gseKEGG.emapplot.pdf"))
        pdf(pdfGseKEGGemap)
        print(emapplot(gsea_kegg2, showCategory=20))
        dev.off()

        pdfGse2KEGG <- file.path(paste0("cxi_interaction_WaldTest-DE", ".geneset.1-", length(gsea_kegg$ID), ".gseKEGG.plot2.pdf"))
        pdf(pdfGse2KEGG)
        print(gseaplot2(gsea_kegg, geneSetID = 1:length(gsea_kegg$ID), title=gsea_kegg$Description[i], pvalue_table = TRUE))
        dev.off()
...
# Save the results to CSV
write.csv(as.data.frame(gsea_go), "cxi_interaction_WaldTest-DE_GSEA_gseGO_results.csv")

## Plot results
# Check if gsea_go contains any results before plotting
if (!is.null(gsea_go) && nrow(gsea_go@result) > 0) {

        pdfGseGO <- file.path(paste0("cxi_interaction_WaldTest-DE_", ".gseGO.dotplot.pdf"))
        pdf(pdfGseGO)
        print(dotplot(gsea_go, showCategory = 20))
        dev.off()

        pdfGseGOridge <- file.path(paste0("cxi_interaction_WaldTest-DE_", ".gseGO.ridgeplot.pdf"))
        pdf(pdfGseGOridge)
        print(ridgeplot(gsea_go))
        dev.off()

        pdfGseGOemap <- file.path(paste0("cxi_interaction_WaldTest-DE_", ".gseGO.emapplot.pdf"))
        pdf(pdfGseGOemap)
        ...
        for (i in seq_along(gsea_go$ID)) {
                pdfGse2GO <- file.path(paste0("cxi_interaction_WaldTest-DE", ".geneset", i, ".gseGO.plot2.pdf"))
                pdf(pdfGse2GO)
                print(gseaplot2(gsea_go, geneSetID = i, title=gsea_go$Description[i]))
                dev.off()

                pdfGseGOcowplot <- file.path(paste0("cxi_interaction_WaldTest-DE", ".geneset", i, ".gseGO.cowplot.pdf"))
                pdf(pdfGseGOcowplot)
        ...
        pdfGse2GO <- file.path(paste0("cxi_interaction_WaldTest-DE", ".geneset.1-", length(gsea_go$ID), ".gseGO.plot2.pdf"))
        pdf(pdfGse2GO)
        ...
        tryCatch({
            pdfGOtreeplot <- file.path(paste0("cxi_interaction_WaldTest-DE", ".gseGO.treeplot.pdf"))
            pdf(pdfGOtreeplot)
            print(treeplot(gsea_go2, cluster.params = list(method = "average")))
            dev.off()

            pdfGseGOemap <- file.path(paste0("cxi_interaction_WaldTest-DE", ".gseGO.emapplot.pdf"))
            pdf(pdfGseGOemap)
            print(emapplot(gsea_go2, showCategory=20))
            dev.off()

            pdfGseGOupset <- file.path(paste0("cxi_interaction_WaldTest-DE", ".gseGO.upset.pdf"))
            pdf(pdfGseGOupset)
